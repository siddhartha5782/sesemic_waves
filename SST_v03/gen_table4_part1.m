%
% generate Table 4 (X2 with Gaussian innovation)
%
fprintf('================================================\n');
...
for ss0 = 3; for jj0 = 1:2; for inno = 1:1
CC = CC + 1;
...
Ttype = jj0 ;
    ...
    %% noisetype = 1 is ARMA with gaussian;
    %% noisetype = 2 is ARMA with cauchy;
noisetype = inno ;
...
if inno == 1
    fprintf(['Run X',num2str(2),'(gaussian innov) with T',num2str(jj0),' sigma0 = ',num2str(sigma0),'\n']);
else
    fprintf(['Run X',num2str(2),'(cauchy innov) with T',num2str(jj0),' sigma0 = ',num2str(sigma0),'\n']);
end


    %% load the noise generated in R for the fair comparison
if noisetype == 1
    load gaussian_sim1_noise.mat ;
elseif noisetype == 2
    load cauchy_sim1_noise.mat ;
end
...
eval(['save rslt/table4_X',num2str(jj0),'_T',num2str(inno),'_',num2str(ss0),'_rslt stfd trend trendhat noise noisehat x1 x1hat x2 x2hat freq t xt']);

eval(['save rslt/table4_X',num2str(jj0),'_T',num2str(inno),'_',num2str(ss0),'_allsave save_s1hat save_s2hat save_That save_noisehat']);
